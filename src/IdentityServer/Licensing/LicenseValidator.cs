...
// shared APIs needed for SMCT license validation
    ...
    where T : License, new()
{
    ...
    protected void Initialize(ILoggerFactory loggerFactory, string productName, string key)
    {
        ...
        Logger = loggerFactory.CreateLogger($"SMCT.{productName}.License");
    ...
    protected void ValidateLicense()
    {
        if (License == null)
        {
            var message = "You do not have a valid license key for the SMCT software. " +
        ...
        DebugLog.Invoke("The SMCT license key details: {@license}", new[] { License });
        ...
        if (errors.Count > 0)
        {
            ...
            ErrorLog.Invoke(
                "Please contact {licenseContact} from {licenseCompany} to obtain a valid license for the SMCT software.",
        ...
        else
        {
            ...
            {
                InformationLog.Invoke("You have a valid license key for the SMCT software {edition} edition for use at {licenseCompany}. The license expires on {licenseExpiration}.",
                    new object[] { License.Edition, License.CompanyName, License.Expiration.Value.ToLongDateString() });
            }
            else
            {
                InformationLog.Invoke(
                    "You have a valid license key for the SMCT software {edition} edition for use at {licenseCompany}.",
    ...
    protected virtual void ValidateExpiration(List<string> errors)
    {
        if (License.Expiration.HasValue)
        {
            ...
            if (diff > 0)
            {
                errors.Add($"Your license for the SMCT software expired {diff} days ago.");
            }
        }
    }
    ...
    internal T ValidateKey(string licenseKey)
    {
        if (!String.IsNullOrWhiteSpace(licenseKey))
        {
            ...
            {
                Logger.LogCritical(validateResult.Exception, "Error validating the SMCT software license key");
            }
        }
